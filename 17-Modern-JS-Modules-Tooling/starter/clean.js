const shoppingCart = [
  { product: 'bread', quantity: 6 },
  { product: 'pizza', quantity: 2 },
  { product: 'milk', quantity: 4 },
  { product: 'water', quantity: 10 },
];

const allowedProducts = {
  lisbon: 5,
  others: 7,
};

const checkAllowedProducts = function (cart, numAllowed, city) {
  // Si el cart no tiene length, o sea 0
  if (!cart.length) return [];

  // const allowed = numAllowed[city] > 0 ? numAllowed[city] : numAllowed.others;
  const allowed = numAllowed?.[city] ?? numAllowed.others; // otra forma de escribir lo de arriba

  const newCart = cart.map(item => {
    const { product, quantity } = item;
    return {
      product,
      quantity: quantity > allowed ? allowed : quantity,
    };
  });

  return newCart;
};

const allowedShoppingCart = checkAllowedProducts(
  shoppingCart,
  allowedProducts,
  'lisbon'
);
console.log(allowedShoppingCart);

const createOrderDescription = function (cart) {
  // const first = cart[0];
  // const p = first.product;
  // const q = first.quantity;

  // const { product, quantity } = cart[0]; // Lo mismo de abajo
  const [{ product: prod, quantity: qt }] = cart;

  return `Order with ${qt} ${prod} ${cart.length > 1 ? 'etc...' : '.'}`;
};

const orderDescription = createOrderDescription(allowedShoppingCart);

console.log(orderDescription);
